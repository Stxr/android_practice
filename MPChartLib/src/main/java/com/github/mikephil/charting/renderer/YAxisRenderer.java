...
public class YAxisRenderer extends AxisRenderer {
    ...
    protected float[] mRenderLabelsBuffer=new float[2];
    /**
...
//            renderLimitRange(c);
    ...
    public void renderLimitRange(Canvas c) {
        List<LimitRange> limitRanges = mYAxis.getLimitRanges();
        for(LimitRange r:limitRanges) {
            String label = r.getLabel();
            mLimitRangePaint.setTextSize(r.getTextSize());
            float offset = Utils.calcTextHeight(mLimitRangePaint, "A") / 2.5f + mYAxis.getYOffset();
            float pos[]=mRenderLabelsBuffer;
            float position = (r.getBottomLimit().getLimit()+r.getTopLimit().getLimit())/2;
            pos[1]=position;
            mTrans.pointValuesToPixel(pos);
            if (mYAxis.getAxisDependency() == AxisDependency.LEFT) {//左边Y轴
                mLimitRangePaint.setTextAlign(Align.RIGHT);
                c.drawText(label, mViewPortHandler.contentLeft() - mYAxis.getXOffset() - r.getXOffset(), pos[1] + offset, mLimitRangePaint);
            } else {//右边Y轴
                mLimitRangePaint.setTextAlign(Align.LEFT);
                c.drawText(label, mViewPortHandler.contentRight() + mYAxis.getXOffset() - r.getXOffset(), pos[1] + offset, mLimitRangePaint);
            }
        }
    }
}
