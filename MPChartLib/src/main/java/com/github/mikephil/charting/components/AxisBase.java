...
public abstract class AxisBase extends ComponentBase {
     ...
     * @param r
     */
    public void addRangeLimitLine(LimitRange r){
        mLimitRanges.add(r);
        //复用LimitLine
        mLimitLines.add(r.getBottomLimit());
        mLimitLines.add(r.getTopLimit());
    }
    ...
    public String getLongestLabel() {
        ...
        for (LimitRange range : mLimitRanges) {
            String text = range.getLabel();
            if (text != null && longest.length() < text.length())
                longest = text;
        }
